...
export function FeedbackScreen({ topicId }: { topicId: TopicId }) {
  ...
  return (
    <form className="screen-content feedback-content" onSubmit={submit} aria-labelledby="feedback-title">
      ...
      <label className="feedback-detail-field">상세 사유<select value={reason} onChange={(event) => setReason(event.target.value)}><option value="">선택하지 않음</option><option value="구체적이고 이해하기 쉬워요">구체적이고 이해하기 쉬워요</option><option value="내용이 너무 일반적이에요">내용이 너무 일반적이에요</option><option value="내 경험과 맞지 않아요">내 경험과 맞지 않아요</option><option value="표현이 불편하거나 단정적이에요">표현이 불편하거나 단정적이에요</option></select></label>
