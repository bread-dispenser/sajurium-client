...
const muted: CSSProperties = { color: "var(--stone-700)", fontSize: 12, lineHeight: 1.55 };
...
export function AdminOperationsPrototypeScreen() {
  ...
  return <main className="screen-content" aria-labelledby="operations-title" style={{ gap: 24 }}>
    ...
    <aside style={{ ...panel, borderColor: "rgb(169 52 34 / 35%)", background: "var(--surface-warm)" }}><strong>비운영 데모</strong><p style={{ ...muted, marginTop: 4 }}>버튼은 이 탭의 React 상태와 아래 감사 표시만 바꿉니다. 실제 관리자 권한이나 민감정보 접근 권한을 제공하지 않습니다.</p></aside>
    ...
    <nav data-slop-allow="overstuffed-row" role="tablist" aria-label="운영 업무" style={{ display: "flex", gap: 7, overflowX: "auto", paddingBottom: 4 }}>
      {(Object.keys(TAB_LABELS) as OperationTab[]).map((id) => <button key={id} type="button" role="tab" aria-selected={tab === id} onClick={() => changeTab(id)} style={{ minWidth: 112, minHeight: 44, border: tab === id ? "1px solid var(--vermilion)" : "var(--rule)", borderRadius: 999, background: tab === id ? "rgb(169 52 34 / 8%)" : "var(--surface-strong)", color: tab === id ? "var(--vermilion-dark)" : "var(--ink)", fontWeight: 750 }}>{TAB_LABELS[id]}</button>)}
    </nav>
    ...
    <section data-slop-allow="overstuffed-row" aria-labelledby="records-title" style={{ display: "grid", gap: 12 }}>
      ...
      <div data-slop-allow="overstuffed-row" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: 12 }}>
        <div style={{ display: "grid", gap: 8, alignContent: "start" }}>
          {rows.length ? rows.map((item) => <button key={item.id} type="button" onClick={() => { setSelectedId(item.id); setPending(null); setMessage(`${item.id} 상세 예시를 선택했습니다.`); }} aria-pressed={selected?.id === item.id} style={{ ...panel, textAlign: "left", boxShadow: selected?.id === item.id ? "inset 3px 0 var(--vermilion)" : "none" }}><small style={{ color: "var(--vermilion)", fontWeight: 800 }}>{item.id}</small><strong style={{ display: "block", margin: "5px 0" }}>{item.title}</strong><span style={muted}>{item.status}</span></button>) : <div style={panel}><strong>검색 결과가 없습니다.</strong><p style={muted}>예시 ID나 상태로 다시 검색하세요.</p></div>}
        </div>
        <article style={{ ...panel, background: "var(--surface-warm)", alignSelf: "start" }} aria-label="선택 레코드 상세">
          {selected ? <><p className="section-kicker">{selected.id}</p><h2>{selected.title}</h2><p style={{ ...muted, margin: "6px 0 14px" }}>{selected.summary}</p><dl style={{ display: "grid", gap: 9, margin: 0 }}>{selected.facts.map(([term, value]) => <div key={term} style={{ borderTop: "1px solid rgb(32 30 27 / 12%)", paddingTop: 8 }}><dt style={muted}>{term}</dt><dd style={{ margin: 0, fontWeight: 700 }}>{value}</dd></div>)}</dl></> : <p>레코드를 선택하세요.</p>}
        </article>
      </div>
    </section>
...
export function AdminAnalyticsPrototypeScreen() {
  ...
  return <main className="screen-content" aria-labelledby="analytics-title" style={{ gap: 24 }}>
    ...
    <aside style={{ ...panel, background: "var(--surface-warm)", borderColor: "rgb(169 52 34 / 35%)" }}><strong>분석 데이터 아님</strong><p style={{ ...muted, marginTop: 4 }}>필터는 표시 문구만 바꾸며 서버 쿼리를 실행하지 않습니다. 일반 분석 이벤트에는 출생 정보 원문과 상담 내용 전문을 넣지 않는다는 개인정보 원칙을 함께 보여줍니다.</p></aside>
    ...
    <section data-slop-allow="overstuffed-row" aria-labelledby="metrics-title"><div><p className="section-kicker">PRD 핵심 지표 전체</p><h2 id="metrics-title">운영 요약</h2></div><div data-slop-allow="overstuffed-row" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))", gap: 10, marginTop: 10 }}>{METRICS.map(([label, value, note]) => <article key={label} style={panel}><small style={muted}>{label}</small><strong style={{ display: "block", margin: "5px 0", fontFamily: "var(--editorial-font)", fontSize: 25, fontWeight: 500 }}>{value}</strong><span style={{ color: label.includes("실패") || label.includes("오류") || label.includes("신고") ? "var(--vermilion-dark)" : "var(--stone-700)", fontSize: 11 }}>{note}</span></article>)}</div></section>

    <section data-slop-allow="overstuffed-row" aria-labelledby="errors-title" style={{ display: "grid", gap: 10 }}><div><p className="section-kicker">실패·오류율</p><h2 id="errors-title">품질 경보 예시</h2></div><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(190px, 1fr))", gap: 10 }}>{[["원국 계산 실패", "2.0%", "127 / 6,458"], ["리포트 생성 실패", "1.7%", "108 / 6,331"], ["결제 실패", "14.4%", "101 / 702"], ["이용권 차감 오류", "0.08%", "6 / 7,486"], ["안전 필터 제한", "2.4%", "13 / 534"], ["알림 열기 실패", "0.6%", "7건 예시"]].map(([label, value, note]) => <article key={label} style={{ ...panel }}><strong>{label}</strong><span style={{ display: "block", marginTop: 7, fontSize: 22 }}>{value}</span><small style={muted}>{note}</small></article>)}</div></section>
